...
void AppInvoker::openAppOnAppWorld(QString url)
{
	bb::system::InvokeRequest request;
	request.setTarget("sys.appworld");
	request.setAction("bb.action.OPEN");
	//"appworld://content/lemming-MusicPlus"
	request.setUri(url);
	bb::system::InvokeTargetReply* reply = m_invokeManager.invoke(request);
	if (reply)
	{
...
void AppInvoker::openBrowser(QString url)
{
    bb::system::InvokeRequest request;
...
void AppInvoker::shareText(QString text)
{
    bb::system::InvokeRequest request;
    request.setAction("bb.action.SHARE");
    request.setMimeType("text/plain");
    request.setData(text.toLocal8Bit());
